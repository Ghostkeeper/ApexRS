...
mod tests {
	...
	fn multiply_i32(lhs: i32, rhs: i32) {
		...
		let result = kernel_call(&module, "test_multiply_i32", &[lhs.to_le_bytes(), rhs.to_le_bytes()].concat(), 4, 5, 8);
	...
	fn abs_i32(value: i32) {
		...
		let module = GPU.device.create_shader_module(include_wgsl!("../../src/operations/area_polygon.wgsl"));
		let result_bytes = kernel_call(&module, "test_abs_i32", &value.to_le_bytes(), 6, 7, 4);
	...
	fn add(lhs: i64, rhs: i64) {
		...
		let module = GPU.device.create_shader_module(include_wgsl!("../../src/operations/area_polygon.wgsl"));
		let result = kernel_call(&module, "test_add", &combined, 8, 5, 8);
		let emulated_result = EmulatedI64::from(result);
		assert_eq!(using_i64, emulated_result.into());
	}
	...
	fn sub(lhs: i64, rhs: i64) {
		...
		combined.append(&mut emulated_rhs.into());
		let module = GPU.device.create_shader_module(include_wgsl!("../../src/operations/area_polygon.wgsl"));
		let result = kernel_call(&module, "test_sub", &combined, 8, 5, 8);
		let emulated_result = EmulatedI64::from(result);
		assert_eq!(using_i64, emulated_result.into());
	}
	...
	fn neg(value: i64) {
		...
		let module = GPU.device.create_shader_module(include_wgsl!("../../src/operations/area_polygon.wgsl"));
		let result = kernel_call(&module, "test_neg", &emulated_value, 9, 5, 8);
		let emulated_result = EmulatedI64::from(result);
		assert_eq!(using_i64, emulated_result.into());
	}
}