...
/// * `rhs` - The number to subtract from the other number.
fn subtract(lhs: EmulatedF64, rhs: EmulatedF64) -> EmulatedF64 {
...
///
/// # Arguments
/// * `value` - The original value to negate (`x` in the above explanation).
///
/// # Implementation
/// The individual high and low components of this number are negated. This results in no loss of
/// precision, since the sign of the number is stored separately.
fn negate(value: EmulatedF64) -> EmulatedF64 {
	return EmulatedF64(-value.high, -value.low);
}
...
fn main(@builtin(global_invocation_id) global_id: vec3<u32>) {
	...
	vertices[index].x = round(subtract(multiply(x, cosine), multiply(y, sine)));
	vertices[index].y = round(add(multiply(x, sine), multiply(y, cosine)));
}